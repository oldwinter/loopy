...
export const loops = [
  ...
  },
  {
    ...
    seoTitle: "autonomy-loop Adversarial Builder Workflow | Loop Library",
    description:
      "A two-worktree coding loop that pairs a builder with an independent adversarial reviewer and proves each new test can catch the change it protects.",
    categoryLabel: "AI coding agent workflow",
    author: "@inferencegod",
    ...
    verifyTitle: "Every accepted wave passes an independent proof-of-test gate.",
    verifyDetail:
      "The acceptance test fails without the change, passes with it, all configured gates pass, and protected production changes remain human- or forge-gated.",
    useWhen:
      "Use this for a repository task that benefits from continuous builder-reviewer handoffs and has strong deterministic gates plus real infrastructure-level branch protection.",
    steps: [
      "Initialize the plugin, separate builder and reviewer worktrees, configure deterministic gates and frozen invariants, and verify the hardened production rails.",
      "Have the builder implement one bounded change with a red-before-green acceptance test, run the gates, commit, and update the git-backed baton.",
      "Have the reviewer rerun every gate, conduct the adversarial audit, and use revert-or-mutate proof to show that the new test catches the change.",
      "Accept and continue only on independent proof; otherwise return the findings to the builder or park for a human when a circuit breaker fires.",
    ],
    why:
      "The separate worktree and reviewer reduce self-approval, while the revert-or-mutate check catches tests that execute code without proving the fix. A committed baton makes the loop resumable after process or context loss.",
    note:
      "Treat local hooks as tripwires, not a security boundary. Review and pin the plugin version, use server-enforced branch protection and separate principals for unattended promotion, and expect two continuously running agents to consume substantial tokens.",
    keywords: [
      "autonomy-loop",
  ...
  },
  {
    ...
    description:
      "A Codex Goal workflow that defines completion up front, tracks proof for every requirement, and prevents partial or weak evidence from being reported as done.",
    categoryLabel: "AI coding agent workflow",
    ...
    verifyTitle: "Every explicit requirement has current, adequate proof.",
    verifyDetail:
      "The final audit contains no weak, missing, or contradicted required item; otherwise the Goal remains open, blocked, or exhausted rather than complete.",
    useWhen:
      "Use this for long-running Codex work, external state changes, pull requests, runtime checks, or user-visible artifacts where a plausible-looking partial result could be mistaken for completion.",
    steps: [
      "Inspect the active Goal, classify the rigor needed, and recover a measurable definition of done for any ambiguous requirement.",
      "Create the compact Goal, optional durable contract, and live plan without expanding the user's requested scope.",
      "Execute one bounded action at a time and attach current evidence to each affected requirement.",
      "Audit every requirement before closure, completing only fully proved work and preserving honest blocked, exhausted, or contradicted states.",
    ],
    why:
      "A separate completion contract survives long sessions and keeps the definition of done visible. Mapping every requirement to evidence makes false completion easy to detect before the Goal is closed.",
    note:
      "Use native Codex Goal tools only after an explicit Goal request. Evidence must prove the current state, not merely show that a command once ran, and budget exhaustion never counts as successful completion.",
    keywords: [
      "Codex Goal",
      "completion contract",
      "evidence audit",
  ...
  },
  {
    ...
    title: "The Revolve self-improvement loop",
    seoTitle: "Revolve Evidence-Based Self-Improvement Loop | Loop Library",
    description:
      "A revisioned research workflow that checkpoints an incumbent, evaluates comparable candidates, and promotes improvements only with reproducible evidence and rollback.",
    categoryLabel: "AI product evaluation workflow",
    author: "Agent Zero",
    ...
    verifyTitle: "The best checkpoint wins under one reproducible evaluation revision.",
    verifyDetail:
      "The incumbent and candidates have comparable recorded runs, no accepted guard regression, a recoverable rollback path, and no unapproved live-file promotion.",
    useWhen:
      "Use this to improve a prompt, policy, workflow, model configuration, code path, or other testable artifact when experiments need durable research state and comparable evaluation across sessions.",
    steps: [
      "Define the objective and permissions, freeze an evaluation revision, checkpoint the incumbent, and capture a baseline before editing.",
      "Choose one evidence-backed hypothesis, create a recoverable candidate checkpoint, and run it under the unchanged revision.",
      "Record and compare the result, promoting internally only on a meaningful guard-safe win and opening a new revision whenever evaluation comparability changes.",
      "Stop on a named terminal state, and require explicit approval plus post-promotion verification before changing the live artifact.",
    ],
    why:
      "Revision boundaries prevent scores from different tests or rubrics from being compared as if they were equivalent. Checkpoints, branch records, and an internal-before-external promotion boundary keep long-running research resumable and reversible.",
    note:
      "Keep candidate work inside revolve/ until external promotion is explicitly approved. Set the budget and stop directive before searching, and do not improve the evaluator mid-revision to make a preferred candidate win.",
    keywords: [
      "Revolve",
